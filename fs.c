...
stati(struct inode *ip, struct stat *st)
{
  ...
  st->size = ip->size;
	if(ip->type == T_CHECKED) {
    uchar checksum = 0, checksumTemp = 0;
    int i;
    struct buf *bp;
    for(i = 0; i < (NDIRECT + 1); i++) {
      ...
      if(i == NDIRECT) {
        ...
        for(j = 0; j < (BSIZE/sizeof(uint)); j++) {
          checksumTemp ^= (uchar)((a[j] & 0xff000000) >> 24);
        }
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  uint checksum1 = 0, checksumTemp = 0;
  int nbytes;
  if(n > BSIZE)
    nbytes = BSIZE;
  else
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    bp1 = bread(ip->dev, bmap(ip, off/BSIZE));
    ...
    memmove(dst, bp1->data + off%BSIZE, m);
		...
		if(ip->type == T_CHECKED) {
			...
			checksumTemp = (uint)temp;
			if(checksum1 != checksumTemp) {
				brelse(bp1);
				return -1;
			}
		}
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    memmove(bp->data + off%BSIZE, src, m);
      ...
      checksumTemp = (uint) checksum;
      checksumTemp = checksumTemp << 24;

      uint bn = off / BSIZE;
      if(bn < NDIRECT) {
        ip->addrs[bn] &= 0x00ffffff;
        ip->addrs[bn] |= checksumTemp;
      }
